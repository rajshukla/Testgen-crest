...
int main(int argc, char* argv[]) {
  ...
  string prog = argv[1];
  int num_iters = atoi(argv[2]);  //total iterations other than given
  ...
  if(argc>=5){
    ...
    //call the function GivenTestCases
  ...
  if (search_type == "-level"){
    ...
    if (argc <= 5) {
      ...
      strategy = new crest::BoundedDepthFirstSearch(prog, num_iters, atoi(argv[5]));
    }
    ...
    if (argc <= 5) {
      ...
      strategy = new crest::UniformRandomSearch(prog, num_iters, atoi(argv[5]));
    }
  } else {
