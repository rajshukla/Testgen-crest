...
int givenTestFiles=0;  //no of givenTestCases
int givenCounter=0;     //for givenTestCases
bool flagNotGiven=1;  //whether givenTestCases are there or not
float curCoverage=0.0; //coverage percentage after each iteration
bool flagOnce=1; //used to print initial coverage only once
...
void Search::LaunchProgram(const vector<value_t>& inputs) {
  ...
  */
  ...
  int status=system(program_.c_str());
  if(status==-1) fprintf(stderr,"\nERROR in Launch Program\n");
}


void Search::RunProgram(const vector<value_t>& inputs, SymbolicExecution* ex) {
  if(flagNotGiven || givenCounter > givenTestFiles){
      if(flagOnce) {fprintf(stderr, "\n\n\n\n INITIAL COVERAGE:- %0.2f \n\n\n\n",curCoverage); flagOnce=0;}
...
bool Search::UpdateCoverage(const SymbolicExecution& ex,
  ...
  curCoverage=total_num_covered_*100.0/reachable_branches_;

 fprintf(stderr, "Iteration %d (%lds): covered %u branches coverage percentage %0.2f [%u reach fun, %u reach branches].\n",
	  num_iters_, time(NULL)-start_time_, total_num_covered_, curCoverage  ,reachable_functions_, reachable_branches_);

  FILE* fp=fopen("resultAfter.txt","a");
  FILE* fp2=fopen("resultBefore.txt","a");

  if(num_iters_>0){
    fprintf(fp,"%d %u\n",num_iters_,total_num_covered_);
    if(flag) {  fprintf(fp,"%u %u\n",reachable_functions_,reachable_branches_); flag=0;}
  }
  else{
    fprintf(fp2,"%d %u\n",givenCounter-1,total_num_covered_);
    if(flag) {  fprintf(fp2,"%u %u\n",reachable_functions_,reachable_branches_); flag=0;}
    if(givenCounter>givenTestFiles) { flag=1;} 
  }

  if(num_iters_>=max_iters_) fclose(fp);
  if(givenCounter>givenTestFiles) fclose(fp2);
